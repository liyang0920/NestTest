...
public class RecordReader_q19 extends AvroTrevniKeyRecordReader<Record>{
	...
	public RecordReader_q19(List<Schema> schemas) {
...
@Override
	public boolean nextKeyValue() throws IOException, InterruptedException {
		...
		if(!reader.hasNext())
			...
			row++;
			tm = reader.next();
			if(reader.getFileSchema().getName().trim().equals("Part") 
					&& !tm.get("p_brand").toString().equals("Brand#12") 
					&& !tm.get("p_brand").toString().equals("Brand#23") 
					&& !tm.get("p_brand").toString().equals("Brand#34")){
