...
public class RecordReader_q20 extends AvroTrevniKeyRecordReader<Record>{
	...
	public RecordReader_q20(List<Schema> schemas,LongWritable nk) {
...
@Override
	public boolean nextKeyValue() throws IOException, InterruptedException {
		...
		if(!reader.hasNext())
			...
			row++;
			tm = reader.next();
			if(reader.getFileSchema().getName().trim().equals("Part") && (!tm.get("p_name").toString().startsWith("forest"))){
				mCurrentKey.datum(null);
			}else if(reader.getFileSchema().getName().trim().equals("Supplier") && Long.parseLong(tm.get("s_nationkey").toString()) != nk){
				mCurrentKey.datum(null);
			}else{
